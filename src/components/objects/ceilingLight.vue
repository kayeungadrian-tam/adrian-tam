...
const loadModel = async () => {
  ...
  try {
    const model = await loadScaledModel({
      loader: props.loader,
      url: new URL('../../assets/models/ceiling_light.glb', import.meta.url).href,
      position: props.position,
      rotationY: props.rotationY ?? Math.PI,
      targetHeight: props.targetHeight ?? 0.6,
    })
    props.scene.add(model)
    modelRoot.value = model
  } catch (error) {
    console.error('Failed to load ceiling light model.', error)
  } finally {
