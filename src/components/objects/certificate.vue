...
const loadModel = async () => {
  ...
  try {
    const model = await loadScaledModel({
      loader: props.loader,
      url: new URL('../../assets/models/certificate.glb', import.meta.url).href,
      position: props.position,
      rotationY: props.rotationY ?? Math.PI / 2,
      targetHeight: props.targetHeight ?? 0.9,
    })
    props.scene.add(model)
    modelRoot.value = model
  } catch (error) {
    console.error('Failed to load certificate model.', error)
  } finally {
